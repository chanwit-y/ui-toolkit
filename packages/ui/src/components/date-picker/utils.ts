...
export const parseDate = (value?: string, boundary: "start" | "end" = "start") => {
...
export const getRadiusClass = (radius: DatePickerProps["radius"]) => {
	switch (radius) {
		case "none":
			return "rounded-none"
		case "small":
...
export const getSizeClass = (size: DatePickerProps["size"]) => {
	switch (size) {
		case "1":
