...
- **Button navigation** is the additive `ButtonItemConfig.navigation` (`DesignNavigation`: page with fixed `:params`, link, toast, dialog). It exports on the bin as `designNavigation`, never into the engine `ButtonElement`. `LivePreviewModal` plays it with a capture-phase click matched by button label (the engine button carries no id), switching the previewed page (page bar with back / route / page select) or showing the toast/dialog overlay; click-only toasts/dialogs (`trigger: 'button'`) are filtered out of the page.
